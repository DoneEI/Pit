...
import core.command.internal.HashObjectCmd;
 ...
 *
 * @Author: DoneEI
 * @Since: 2021/1/31 7:59 下午
 **/
public class PitIndex {
    public static IndexTree root;
     ...
     * 将暂存区恢复至工作区
     */
    public static void indexTree2WorkingArea() {
        //
    }

    private static void idT2WAHelper(IndexTree indexTree, String filePath) {
        if (indexTree == null || !StringUtils.isNotEmpty(filePath)) {
            return;
        }

        if (StringUtils.equal(indexTree.getNodeType(), IdTNodeEnum.FILE_NODE.getCode())) {
            // 文件类型,则创建并写入
